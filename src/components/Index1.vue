...
<template>
    <div class="back" style="position: relative; padding-bottom: 100px;width:100%; height: auto!important;">
        ...
        <div class="js_dialog" style="opacity: 1;" v-if="isBox3">
            ...
            <div class="weui-dialog" style="padding-bottom: 20px;">
                ...
                <div @click="isBox3 = false" :class="[global.themeType == 'nochange' ? 'isYellow' : 'isBlue' ]"
                     style="width: 62%; border-radius:30px; margin: 10px auto; color: #fff; height: 40px; line-height: 40px;">
                    确定
                </div>
            </div>
    ...
    export default {
        ...
        methods: {
            initFun(){
                ...
                that.devId = that.$utils.getQueryString("devId")
                ...
                this.$axios.post('https://webapi-openapi.aihomi.com/api/v1/oauth/device/info/'+that.pid, [{
                    ...
                    // that.oneData.map(function (item) {
                    //     if(item.lazyCloseTime !=0){
                    //         console.log(item);
                    ...
                    for(let i in list){
                        ...
                        if (that.keyCount == 2){
                            if(i = 2){
                                ...
                                that.getSwitchPic(that.switchDelay3)
                            }
                        }
                        if (that.keyCount == 3){
                            if(i == 2){
                                that.switchDelay2 = list[i].lazyCloseStatus;
                                that.getSwitchPic(that.switchDelay2)
            ...
            // returnSwitchType(value){
            //     let that = this;
            //     if(value == 0){
            //         that.getSwitchPic(value);
            //         value = 1;
            //     }else{
            //         that.getSwitchPic(value);
            //         value = 0;
                    ...
                    onChange: function (result) {
                        ...
                        for (var i = 0; i < itemArray.length; i++) {
                            ...
                            if ($(itemArray[i]).html() === result[0].value.toString() && i<24) {
                                var hourElementStr = "<span id='ele" + i + "' style='font-weight: normal;color:#007DFF'> 时</span>"
                                $(itemArray[i]).append(hourElementStr);
                                $(itemArray[i]).addClass("homi-picker__item")
                            }
                            if ($(itemArray[i]).html() === result[1].value.toString() && i>=24) {
                                var secondElementStr = "<span id='ele" + i + "' style='font-weight: normal;color:#007DFF'> 分</span>"
                    ...
                    onConfirm: function (result) {
                        if(mode == 1){
                            if((result[0].value != 0 && result[1].value != 0 )|| (result[0].value != 0 && result[1].value == 0) || (result[0].value == 0 && result[1].value != 0)) {
                                ...
                                if (that.leftTimeValue != 0) {
                                    ...
                                    console.log("keyCount",that.keyCount)
                                    console.log("key",key)
                                    console.log("rightDelayTimeValue",that.rightDelayTimeValue)
                                }
                            }
                        }
                        ...
                        if(mode == 2){
                            if((result[0].value != 0 && result[1].value != 0 )|| (result[0].value != 0 && result[1].value == 0) || (result[0].value == 0 && result[1].value != 0)) {
                                ...
                                that.leftTimeValue1 = "00:"+ that.$utils.formatdelay1(false, result[0].value*60 + result[1].value*1)

                                var addTime2 = +(result[0].value*60 + result[1].value*1);
                                console.log(result[0].value)
                                console.log(result[0].value*60)
                                console.log(addTime2)
                                console.log(result[1].value)
                                console.log(that.leftTimeValue1+"fanhui")
                                if (that.leftTimeValue1 != 0) {
                                    ...
                                    // console.log()
                                    // inData.overtimeOffTime =  +(result[0].value*60 + result[1].value)
                                    console.log(inData)
                                    that.postData(inData,6)
                                    if(key == 1){
                                        that.leftOvertimeVale = addTime2
                                    }
                                    if(key == 2 && that.keyCount == 3){
                                        that.midOvertimeVale = addTime2
                                    }
                                    if(key == 3 && that.keyCount == 2 || key == 3 && that.keyCount == 3){
                                        that.rightOvertimeVale = addTime2
                                    }
                                }
                            }
                        }
                        ...
                        if(data.mode == 2){
                            if(data.key == 1){
                                if(that.switchOver1 == 0) {
                                    pickerBody.style.opacity = "0.9";
                                    that.getSwitchPic(that.switchOver1);
                                    postSwitchData.changeKey = 1;
                            ...
                            if(data.key == 2){
                                if(that.switchOver2 == 0) {
                                    pickerBody.style.opacity = "0.9";
                                    that.getSwitchPic(that.switchOver2);
                                    postSwitchData.changeKey = 2;
                                    postSwitchData.overTimeOffEnable = 0;
                                    that.postData(postSwitchData,7)
                                    that.switchOver2 = 1;
                            ...
                            if(data.key == 3){
                                if(that.switchOver3 == 0) {
                                    pickerBody.style.opacity = "0.9";
                                    that.getSwitchPic(that.switchOver3);
                                    postSwitchData.changeKey = 3;
                                    postSwitchData.overTimeOffEnable = 0;
                                    that.postData(postSwitchData,7)
                                    that.switchOver3 = 1;
